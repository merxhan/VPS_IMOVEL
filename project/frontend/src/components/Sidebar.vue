...
<template>
  ...
  <aside 
    v-if="token"
    :class="[
      'hidden md:flex bg-inmo-brand text-white min-h-screen flex-col justify-between shadow-lg shrink-0 transition-all duration-300',
  ...
  >
    ...
    <div>
      <div class="p-4 border-b border-slate-700/50 flex items-center h-20" :class="isCollapsed ? 'justify-center' : 'justify-between'">
        <div class="flex items-center space-x-3 overflow-hidden" v-if="!isCollapsed">
          <div class="w-8 h-8 rounded bg-goiania-green flex items-center justify-center text-goiania-ipe font-bold text-lg shrink-0">
            S
          </div>
          <div class="whitespace-nowrap">
            <h1 class="text-lg font-bold tracking-wide">SICI</h1>
            <span class="text-xs text-neutral-muted block">Goiânia - GO</span>
          </div>
        </div>
      ...
      <nav class="mt-6 px-3 space-y-2">
        <button
          ...
          :class="[
            ...
            router.currentRoute.value.name === 'dashboard' 
              ? 'bg-goiania-green text-white shadow-md' 
        ...
        >
          ...
          <span v-if="!isCollapsed" class="whitespace-nowrap">{{ $t('nav.dashboard') }}</span>
        </button>

        <button
          @click="handleNav('properties')"
          :title="isCollapsed ? $t('nav.properties') : ''"
          :class="[
            'w-full flex items-center py-3 rounded-lg text-sm font-medium transition-all duration-150',
        ...
        >
          ...
          <span v-if="!isCollapsed" class="whitespace-nowrap">{{ $t('nav.properties') }}</span>
        </button>

        <button
          @click="handleNav('tenants')"
          :title="isCollapsed ? $t('nav.tenants') : ''"
          :class="[
            'w-full flex items-center py-3 rounded-lg text-sm font-medium transition-all duration-150',
        ...
        >
          ...
          <span v-if="!isCollapsed" class="whitespace-nowrap">{{ $t('nav.tenants') }}</span>
        </button>
        ...
        >
          ...
          <span v-if="!isCollapsed" class="whitespace-nowrap">Usuários</span>
        </button>
      </nav>
    </div>
    ...
    <div class="p-4 border-t border-slate-700/50 bg-slate-900/40">
      <div v-if="!isCollapsed" class="flex items-center space-x-3 mb-3">
        <div class="w-9 h-9 rounded-full bg-slate-700 flex items-center justify-center font-bold text-sm text-goiania-ipe shrink-0">
      ...
      <div v-else class="flex justify-center mb-3">
        <div class="w-9 h-9 rounded-full bg-slate-700 flex items-center justify-center font-bold text-sm text-goiania-ipe shrink-0" :title="user?.name || 'Administrador'">
          {{ user?.name?.substring(0, 2).toUpperCase() || 'AD' }}
        </div>
      </div>
      <button
      ...
      >
        ...
        <span v-if="!isCollapsed" class="whitespace-nowrap">{{ $t('nav.logout') }}</span>
      </button>
    </div>
  </aside>
  ...
  <teleport to="body">
    ...
    <transition name="slide-right">
      <aside 
        v-if="token && mobileOpen"
        class="md:hidden fixed inset-y-0 right-0 w-72 bg-inmo-brand text-white z-50 flex flex-col justify-between shadow-2xl"
      >
        <!-- Brand / Header -->
        <div>
          <div class="p-5 border-b border-slate-700/50 flex items-center justify-between">
            <div class="flex items-center space-x-3">
              <div class="w-8 h-8 rounded bg-goiania-green flex items-center justify-center text-goiania-ipe font-bold text-lg">
                S
              </div>
              <div>
                <h1 class="text-lg font-bold tracking-wide">SICI</h1>
