...
const activeContracts = computed(() => tenants.value.filter(t => t.propertyId).length);
...
<template>
  <div class="p-4 sm:p-6 md:p-8 max-w-[1440px] mx-auto space-y-8 relative">
          ...
          >
            ...
            <td class="p-4">
              <div v-if="tenant.property" class="max-w-[200px]">
                <div class="font-semibold text-sm text-slate-900 truncate" :title="tenant.property.name">
                  {{ tenant.property.name }}
                </div>
                <div class="text-[11px] text-slate-500 truncate" :title="tenant.property.address">
    ...
    <!-- Modal Form (Add / Edit Tenant) -->
    <div 
      v-if="showFormModal"
      class="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-50 flex items-center justify-center p-4"
    >
      <div class="bg-white border border-slate-200 rounded-xl w-full max-w-2xl shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200 flex flex-col">
        <!-- Modal Header -->
        <div class="p-6 border-b border-slate-200 flex justify-between items-center bg-slate-50">
          <div>
            ...
            <p class="text-xs text-slate-500 mt-1 font-sans">Preencha o formulário com os dados fiscais obrigatórios.</p>
          </div>
        ...
        <!-- Modal Form Content -->
        <form @submit.prevent="handleSubmit" class="p-6 space-y-6 flex-1">
          <!-- Form General Errors -->
          <div v-if="formError" class="p-4 bg-red-50 border border-red-200 text-red-700 text-sm rounded-lg flex items-center gap-2">
          ...
          <!-- Property Selection -->
          <div class="space-y-1.5">
            <label class="block text-xs font-bold text-slate-500 uppercase tracking-wider font-sans">Imóvel Vinculado</label>
            <select 
              v-model="formPropertyId"
              class="w-full p-3 bg-slate-50 rounded-lg border border-slate-200 focus:bg-white focus:border-slate-900 focus:ring-0 text-sm text-slate-900 font-medium transition font-sans appearance-none"
            >
              <option value="">Selecione um imóvel do portfólio...</option>
              <option 
          ...
          <div class="p-4 bg-slate-50 border border-slate-200 rounded-lg flex gap-3">
            <svg class="w-5 h-5 text-slate-500 shrink-0 mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
              <path stroke-linecap="round" stroke-linejoin="round" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
            </svg>
            <p class="text-xs text-slate-600 font-sans leading-relaxed">
              Nota: A validação fiscal é obrigatória para a geração de contratos e faturamento automático.
            </p>
          </div>
    ...
    >
      <div class="bg-white border-l border-slate-200 w-full max-w-md h-full p-6 shadow-xl flex flex-col justify-between animate-slide-in">
        <div>
          <div class="flex justify-between items-center border-b border-slate-200 pb-4 mb-6">
            <div>
              <h2 class="text-base font-bold text-slate-900 font-sans">Garantias e Contrato Assinado</h2>
              <span class="text-xs text-slate-500 block font-sans truncate max-w-[300px] mt-0.5" :title="selectedTenant?.name">
                {{ selectedTenant?.name }}
              </span>
            </div>
            <button @click="showDocDrawer = false" class="text-slate-400 hover:text-red-500 font-bold p-1 rounded hover:bg-slate-100 transition">✕</button>
          </div>

          <!-- Document Upload Box -->
          <div class="p-4 bg-slate-50 border border-slate-200 rounded-lg space-y-3 mb-6">
            <label class="block text-xs font-bold text-slate-900 uppercase tracking-wider font-sans">Carregar Contrato ou Identificação PDF</label>
            <input 
              ref="fileInput"
              type="file" 
              accept="application/pdf"
              class="w-full text-xs text-slate-500 file:mr-4 file:py-1.5 file:px-3 file:rounded file:border-0 file:text-xs file:font-semibold file:bg-slate-200 file:text-slate-700 hover:file:bg-slate-300 transition"
            />
            <div class="flex justify-between items-center pt-2">
              <span class="text-[10px] text-slate-400 font-sans">Somente PDF, limite 5MB</span>
              <button 
                @click="handleDocUpload"
                :disabled="docUploading"
                class="px-4 py-1.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded text-xs font-semibold shadow-sm transition flex items-center gap-1 disabled:opacity-50"
              >
          ...
          <div class="space-y-3">
            <h3 class="text-xs uppercase font-bold text-slate-400 tracking-wider font-sans">Documentação Registrada</h3>
            <div v-if="selectedTenant?.documents.length === 0" class="text-sm text-slate-500 py-8 text-center bg-slate-50/50 rounded border border-slate-100 font-sans italic">
              Sem documentos carregados.
            </div>
            <div 
              v-for="doc in selectedTenant?.documents"
              :key="doc.id"
              class="p-3 border border-slate-200 rounded-lg flex items-center justify-between hover:bg-slate-50 transition"
            >
              <div class="flex items-center gap-3 overflow-hidden">
                ...
                <div class="overflow-hidden">
                  <div class="text-xs font-semibold text-slate-900 truncate max-w-[200px]" :title="doc.name">{{ doc.name }}</div>
                  <div class="text-[10px] text-slate-400 font-sans mt-0.5">{{ Math.round((doc.fileSize / 1024) * 10) / 10 }} KB</div>
                </div>
              </div>
              <a 
                :href="getDocDownloadUrl(doc.filePath)"
                target="_blank"
                class="px-2.5 py-1.5 border border-blue-500 text-blue-500 hover:bg-blue-500 hover:text-white rounded text-xs font-semibold transition font-sans"
              >
                Abrir PDF
              </a>
            </div>
        ...
        <button 
          ...
          class="w-full py-3 bg-white border border-slate-200 text-slate-500 hover:text-slate-950 hover:bg-slate-50 rounded text-sm font-semibold transition mt-6 font-sans"
        >
          Fechar Painel
        </button>
      </div>
